              ...
              <Dialog open={isUrlParameterDialogOpen} onOpenChange={setIsUrlParameterDialogOpen}>
                ...
                <Dialog open={isImportDialogOpen} onOpenChange={setIsImportDialogOpen}>
                  ...
                  <DialogContent className="max-w-2xl">
                    <DialogHeader>
                      <DialogTitle>Import Parameters from Integrations</DialogTitle>
                      <DialogDescription>
                        Select parameters from Integrations &gt; URL Parameters to add to this campaign
                      </DialogDescription>
                    </DialogHeader>
                        ...
                        <>
                          <div className="max-h-96 overflow-y-auto">
                            <div className="space-y-2">
                              {allUrlParameters
                                ...
                                .map(param => (
                                  <div key={param.id} className="flex items-center space-x-3 p-3 border rounded-lg hover:bg-gray-50">
                                    <input
                                      type="checkbox"
                                      id={`param-${param.id}`}
                                      ...
                                      onChange={(e) => {
                                        if (e.target.checked) {
                                          setSelectedGlobalParameters(prev => [...prev, param.id]);
                                        } else {
                                          setSelectedGlobalParameters(prev => prev.filter(id => id !== param.id));
                                        }
                                      }}
                                      className="w-4 h-4"
                                    />
                                    <label htmlFor={`param-${param.id}`} className="flex-1 cursor-pointer">
                                      <div className="flex items-center justify-between">
                                        <div>
                                          <div className="flex items-center space-x-2">
                                            <Code className="h-4 w-4 text-purple-500" />
                                            <span className="font-medium">{param.parameterName}</span>
                                            <Badge variant="outline" className="text-xs">{param.parameterType}</Badge>
                                            {param.isRequired && <Badge variant="destructive" className="text-xs">Required</Badge>}
                                          </div>
                                          <div className="text-sm text-gray-500 mt-1">
                                            Report: {param.reportName} • Menu: {param.reportingMenuName}
                                          </div>
                                          {param.description && (
                          ...
                          {allUrlParameters.filter(param => !campaignParameterIds.includes(param.id)).length === 0 && (
                            <div className="text-center py-8">
                              ...
                              <h3 className="text-lg font-medium mb-2">No Parameters to Import</h3>
                              <p className="text-gray-600">All available parameters from Integrations are already assigned to this campaign.</p>
                            </div>
                          )}
                          ...
                          <div className="flex justify-center space-x-3 pt-4 border-t">
                            <Button
                              onClick={() => {
                                if (selectedGlobalParameters.length > 0) {
                                  const newIds = [...campaignParameterIds, ...selectedGlobalParameters];
                                  saveCampaignParameterIds(newIds);
                                  setSelectedGlobalParameters([]);
                                  setIsImportDialogOpen(false);
                                  toast({
                                    title: "Success",
                                    description: `${selectedGlobalParameters.length} parameter(s) imported successfully`
                                  });
                                }
                              }}
                              ...
                              className="px-8"
                            >
                              Import Selected ({selectedGlobalParameters.length})
                            </Button>
                            <Button
                              variant="outline"
                              onClick={() => {
